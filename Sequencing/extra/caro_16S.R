...
tax_table_fp = 'seqtab_wTax_mctoolsr_16s_caro_noCRC.txt'
map_fp = 'caro_16s_mapping_noCRC.txt' 
...
# Variation Explained
eigenvals(pcoa)/sum(eigenvals(pcoa)) # 26.6, 24.1 % variation explained
...
ggplot(input_rar$map_loaded, aes(Axis01, Axis02, colour = combo_label)) +
  ...
  labs(x = "PC1: 26.6% Variation Explained", 
       y = "PC2: 24.1% Variation Explained",
       colour = "Soil Type") +
  scale_colour_viridis_d() +
...
taxa_summary_by_sample_type(tax_sum_families, 
                            ...
                            test_type = 'KW') # Bonferroni corection important since you did many tests at once
